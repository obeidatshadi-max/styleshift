    ...
    <div style={{ position:'relative', zIndex:1, maxWidth:1040, margin:'0 auto', padding:14 }}>
      <div style={{ background:'linear-gradient(180deg,var(--panel),#0a1430)', border:'1px solid var(--line)', borderRadius:16, padding:16, boxShadow:'0 12px 40px rgba(0,0,0,.45)' }}>
        <div style={{ textAlign:'center', padding:'6px 0 14px' }}>
          ...
          <div style={{ fontFamily:'var(--mono)', fontSize:11, letterSpacing:'.25em', textTransform:'uppercase', color:'var(--ink-dim)' }}>Level {level} accuracy · {got}/{total} optimal moves</div>
        </div>
        {level === 4 && meters && (
          <>
            ...
            <div style={{ marginBottom:14, borderRadius:12, padding:'13px 14px', fontSize:13.5, lineHeight:1.5, border:`1px solid ${apexClear?'var(--green)':'var(--red)'}`, background: apexClear?'rgba(62,224,143,.1)':'rgba(255,93,108,.1)' }}>
              <b style={{ display:'block', fontFamily:'var(--mono)', letterSpacing:'.1em', textTransform:'uppercase', fontSize:11, marginBottom:5, color: apexClear?'var(--green)':'var(--red)' }}>{apexClear ? 'Deal balanced' : 'Deal unbalanced'}</b>
              Target to clear: Quota ≥ 60, Morale ≥ 50, Risk ≤ 50.
            </div>
          </>
        )}
        <div style={{ fontFamily:'var(--mono)', fontSize:13, color:'var(--ink-dim)', letterSpacing:'.04em', marginBottom:10 }}>Confidence self-assessment — how ready do you feel handling this style live?</div>
        <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
          ...
          <span style={{ fontFamily:'var(--mono)', fontSize:18, color:'var(--cyan)', width:48, textAlign:'right' as const }}>{conf}%</span>
        </div>
