...
export default function VoicePartnerClosing({ doctor, onDone }: Props) {
  ...
  const { phase, errorKind, result, previewUrl, startRecording, stopRecording, confirmRecording, rerecord, reset } = useVoicePartnerClosing(doctor.id, lang)
  ...
  const checklistHtml = result
    ? `<div>${escapeHtml(result.doctorText)}</div>` +
      ...
      `</ul>` +
      `<div style="margin-top:8px;font-family:var(--mono);font-size:11px;color:var(--ink-dim)">${escapeHtml(t('voiceClosing.durationLabel', { n: String(result.durationSec) }))}</div>`
    : ''
